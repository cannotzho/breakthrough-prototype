...
export default function CombatScreen({ encounterId, chosenWorldDeck, preShields = [], personalDeck, addToCompendium, onEnd }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a1a] relative">
      ...
      {/* Shield reveal modal — shown when the player breaks an opponent shield (#111) */}
      {state.revealedShieldCard && CARDS[state.revealedShieldCard] && (() => {
        ...
        return (
          <div className="absolute inset-0 z-[60] bg-black/70 flex items-center justify-center p-6">
            <div
              className="bg-[#0d1625] border border-[#4ecca3]/30 rounded-xl shadow-2xl flex flex-col items-center p-6 max-w-xs w-full"
              onClick={e => e.stopPropagation()}
            >
              <p className="text-[#e94560] text-xs uppercase tracking-[0.25em] mb-4 font-mono">Shield Broken</p>

              {/* Card at 1.5× scale — compact to fit windowed dialog */}
              <div style={{ transform: 'scale(1.5)', transformOrigin: 'center top', marginBottom: 70 }}>
                <CardComponent card={revealed} />
              </div>

              {/* Lore panel — shows narrative description only, no combat effect */}
              <div className="bg-[#111b2e] border border-[#1e2a40] rounded-lg p-4 text-left w-full">
                <p className="text-[#4ecca3] text-sm font-bold font-mono mb-2">{revealed.name}</p>
                {revealed.flavorText && (
                  <p className="text-[#bbb] text-sm leading-relaxed italic">{revealed.flavorText}</p>
                )}
              </div>

              <button
                onClick={dismissReveal}
                className="mt-4 px-8 py-2.5 bg-[#4ecca3] text-black rounded font-bold font-mono text-base hover:bg-[#3db892] transition-colors"
              >
                Understood
              </button>
            </div>
      ...
      {state.awaitingBackOfMindChoice && botmPickerReady && !state.gameOver && (
        <BackOfMindPicker
          ...
          limit={state.combatConfig.backOfMindLimit}
        />
      )}
